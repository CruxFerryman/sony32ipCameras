...
def convertPanToHex(inputdegrees):
    ...
    if (inputdegrees < min_pan_degrees):
        inputdegrees += 360
    signedIntVal = int(inputdegrees / degrees_per_hex_val)
    if (signedIntVal < 0):
...
def continous_roll(ip_address, pan_values_list, tilt_values_list):
    ...
    for status_number in range(file_number):
        newpan = float(pan_values_list[status_number])
        newtilt = float(tilt_values_list[status_number])
        status_ip = 'http://' + ip_address + '/command/' + status_filename
        r_status = requests.get(status_ip, auth=('admin', 'admin'))
        nowpan = convertPanToDegrees(r_status.text[13:17])
        pan_sub = abs(float(newpan) - float(nowpan))
        velocity = int(pan_sub)
        if velocity > 10:
            velocity = 10
        elif velocity <= 5:
            velocity = 6
        new_pan_hex = convertPanToHex(newpan)
        new_tilt_hex = convertTiltToHex(newtilt)
        print(velocity)
        r = requests.get(url + str(new_pan_hex) + ',' + str(new_tilt_hex) + ',' + str(velocity),
                         auth=('admin', 'admin'))
        print(ip_address, status_number, r)
    sleep(8)
    for status_number in range(file_number - 2, 0, -1):
        newpan = float(pan_values_list[status_number])
        newtilt = float(tilt_values_list[status_number])
        status_ip = 'http://' + ip_address + '/command/' + status_filename
        r_status = requests.get(status_ip, auth=('admin', 'admin'))
    ...
    while True:
        ...
        for ip in ip_table:
            ...
            for status_num in range(file_number):
                print(status_num, index, p[status_num][index], t[status_num][index])
                p_ip_values.append(p[status_num][index])
                t_ip_values.append(t[status_num][index])
        ...
        # for status_number in range(file_number):
        #     pool_ip = Pool(len(ip_table))
        #     p_status = p[status_number]
        #     t_status = t[status_number]
        #     print("The status_number is " + str(status_number + 1))
        #     for ip_num in range(len(ip_table)):
        #         ip = ip_table[ip_num]
        #         p_value = p_status[ip_num]
        #         t_value = t_status[ip_num]
        #         # print('Creating subprocess for ' + str(ip) + ', status=' + str(status_number + 1) + ', pan=' + str(p_value) + ', tilt=' + str(t_value))
        #         pool_ip.apply_async(hexchangePT, args=(ip, p_value, t_value, ))
        #     logging.info('Waiting for all subprocesses done...')
        #     pool_ip.close()
        #     pool_ip.join()
        #     logging.info('All subprocesses done.')
        # for status_number in range(file_number - 1, -1, -1):
        #     pool_ip = Pool(len(ip_table))
        #     p_status = p[status_number]
        #     t_status = t[status_number]
        #     print("The status_number is " + str(status_number + 1))
